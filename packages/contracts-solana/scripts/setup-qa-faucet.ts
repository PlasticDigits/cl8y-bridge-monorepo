...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(qaPath)) {
    throw new Error(
      `Missing ${qaPath} — run registerAllTokens / qa:full-token-setup order`
    );
  }
  ...
  if (!cfgInfo) {
    ...
    console.log(
      "[setup-qa-faucet] Faucet config already exists — skip initialize"
    );
  }
  ...
  for (const ms of mintStrs) {
    ...
    if (!auth.equals(kp.publicKey)) {
      console.warn(
        `[setup-qa-faucet] skip ${ms}: mint authority ${auth.toBase58()} is not admin`
      );
      continue;
    }
