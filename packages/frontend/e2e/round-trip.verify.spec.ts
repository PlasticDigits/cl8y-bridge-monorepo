...
test.describe('Round-Trip Transfer Verification', () => {
  ...
  test('Anvil -> Anvil1 -> Anvil round-trip', async ({ connectedPage: page }) => {
    ...
    const nonce1 = getDepositNonceFromReceipt(ANVIL_RPC, depositTx1)
    console.log(`[round-trip] Leg 1 nonce: ${nonce1}`)

    // WithdrawSubmit on Anvil1
    ...
    // Compute hash and poll for approval
    ...
    // Skip cancel window and wait for execution
    await skipAnvilTime(ANVIL1_RPC, 600)
    const executed1 = await pollForExecution(ANVIL1_RPC, bridge1Address, hash1, 30_000)
    console.log(`[round-trip] Leg 1 executed: ${executed1}`)
    ...
    const nonce2 = getDepositNonceFromReceipt(ANVIL1_RPC, depositTx2)
    console.log(`[round-trip] Leg 2 nonce: ${nonce2}`)

    // WithdrawSubmit on Anvil
    ...
    const executed2 = await pollForExecution(ANVIL_RPC, bridgeAddress, hash2, 30_000)
    console.log(`[round-trip] Leg 2 executed: ${executed2}`)
