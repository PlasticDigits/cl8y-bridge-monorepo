...
export async function queryEvmDeposit(
  ...
  try {
    const [deposit, thisChainId] = await Promise.all([
      client.readContract({
 ...
 * Uses getThisChainId for destChain to match the contract's hash computation.
 */
export async function queryEvmPendingWithdraw(
  ...
  try {
    const [pendingWithdraw, cancelWindow, block, thisChainId] = await Promise.all([
      client.readContract({
        address: bridgeAddress,
        abi: BRIDGE_VIEW_ABI,
    ...
    const destChainHex = bytes4ToBytes32(thisChainId as `0x${string}`)
    const tokenBytes = evmAddressToBytes32(pendingWithdraw.token as Address)

    const approved = pendingWithdraw.approved
    const cancelled = pendingWithdraw.cancelled
    const windowEnd = Number(pendingWithdraw.approvedAt) + Number(cancelWindow ?? 0)
    const now = block?.timestamp ?? 0
    const cancelWindowRemaining =
      approved && !cancelled ? Math.max(0, windowEnd - Number(now)) : undefined
