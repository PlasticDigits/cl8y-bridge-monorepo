 ...
 * Deploys 3 test tokens (TokenA, TokenB, TokenC) + LUNC (uluna representation) across all 3 chains.
 */

import { deployThreeTokens, deployLuncToken } from './deploy-evm'
import { deployThreeCw20Tokens } from './deploy-terra'

export interface TokenAddresses {
  anvil: { tokenA: string; tokenB: string; tokenC: string; lunc: string }
  anvil1: { tokenA: string; tokenB: string; tokenC: string; lunc: string }
  terra: { tokenA: string; tokenB: string; tokenC: string }
}

/**
 * Deploy 3 tokens + LUNC to all 3 chains.
