...
|----------|-------|---------|
| High | 0 | — (cancel window fixed) |
| Medium | 0 | — (RBAC deviations fixed) |
...
|-------------|-------|--------|
| Withdraw flow (V2) | ~30+ tests | ✅ Excellent – full lifecycle, cancel/uncancel, edge cases |
...
| Incoming token registry | Multiple | ✅ Good – mapping, validation |
| Fee system | Multiple | ✅ Adequate |
| Hash parity | Multiple | ✅ Excellent – cross-chain hash consistency |
| Address codec | Unit tests | ✅ Adequate |
| Outgoing (deposit) | Limited | ⚠️ Partial – `test_lock_stores_deposit_hash` only |

### 1.3 Test Coverage Gaps

1. **Deposit flows under-tested** – No comprehensive tests for `DepositNative` min/max limits, fee calculation, rate limits, or CW20 deposit (lock/burn) with edge cases.

2. **Admin/recovery flows** – No tests for `RecoverAsset`, `ProposeAdmin`/`AcceptAdmin` timelock, or paused-state behavior beyond basic pause checks.

3. **Rate limit enforcement** – Tests configure rate limits but do not verify that withdrawals actually enforce per-transaction and per-period limits in `WithdrawExecuteUnlock`/`WithdrawExecuteMint`.

4. **MintBurn execute** – `test_execute_unlock_wrong_token_type_rejected` validates LockUnlock vs MintBurn, but there is no full-flow test for `WithdrawExecuteMint` with actual CW20 mint.

5. **Operator authorization** – Tests verify operator can approve and user cannot; no negative tests for canceler-only scenarios (e.g., operator trying to cancel when design says only canceler).

6. **Migration** – No tests for `migrate` entry point (v1→v2 WITHDRAW_DELAY and FEE_CONFIG initialization).
...
| Public | — | Deposit, Withdraw (submit + execute) |
...
| RegisterChain, AddToken, etc. | Admin | Admin | ✅ |
| AddCanceler / RemoveCanceler | Admin | Admin | ✅ |
| SetWithdrawDelay, SetRateLimit | Admin | Admin | ✅ |
| AddOperator / RemoveOperator | Admin | Admin | ✅ |
| UpdateLimits, UpdateFees, SetFeeParams | Admin | Admin | ✅ |
| **WithdrawApprove** | Operator only | Operator **or Admin** | ⚠️ Operator + Admin |
| **WithdrawUncancel** | Operator only | Operator **or Admin** | ⚠️ Operator + Admin |
| **WithdrawCancel** | Canceler only | Canceler only | ✅ Fixed |
| **SetCustomAccountFee** | Admin | Admin only | ✅ Fixed |
| **RemoveCustomAccountFee** | Admin | Admin only | ✅ Fixed |
| DepositNative, Receive | Public | Public | ✅ |
| WithdrawSubmit | Public | Public | ✅ |
| WithdrawExecuteUnlock / Mint | Public | Public | ✅ |

### 2.3 RBAC Findings

1. **WithdrawApprove / WithdrawUncancel allow Admin** – Design states operator *solely* for these. Allowing admin is a convenience but expands the operator-equivalent trust surface. **Recommendation:** Document as intentional (admin can act as backup operator) or restrict to operators only.

2. **WithdrawCancel** – Now restricted to cancelers only. Operators and admin cannot cancel.

3. **SetCustomAccountFee / RemoveCustomAccountFee** – Now restricted to admin only.
...
- No rate limit on deposits (only on withdrawals). ⚠️ Consider symmetric limits if desired.

### 3.2 Withdrawal Flow (V2 – Incoming)

1. **WithdrawSubmit** – User submits params (src_chain, src_account, token, recipient, amount, nonce). Creates `PendingWithdraw`; optional operator gas tip.
2. **WithdrawApprove** – Operator (or admin) approves; marks nonce used; starts cancel window; transfers operator gas to approver.
3. **WithdrawCancel** – Canceler (or operator or admin) cancels within cancel window.
4. **WithdrawUncancel** – Operator (or admin) restores cancelled withdrawal and resets cancel window.
5. **WithdrawExecuteUnlock / WithdrawExecuteMint** – Anyone executes after cancel window; releases tokens (unlock or mint).

**Observations:**
- Nonce reuse prevented via `WITHDRAW_NONCE_USED`. ✅
- Cancel window enforced before execute. ✅
- Rate limits applied at execution. ✅
...
**Verdict:** ✅ Reentrancy risk is low in this architecture.

### 4.2 Integer Overflow / Underflow

- Uses `Uint128` and `checked_mul` where applicable (e.g., `normalize_decimals`).
- One `unwrap_or(Uint128::MAX)` in `normalize_decimals` when scaling up—edge case if `amount * multiplier` overflows; consider `checked_mul` and explicit error.
...
**Verdict:** ⚠️ Inherent to bridge design; document and monitor.

### 4.6 Input Validation

- Chain IDs: 4 bytes; `0x00000000` rejected. ✅
- Token mappings validated. ✅
- Hash lengths: 32 bytes for withdraw_hash. ✅
...
### 5.3 MintBurn Execution – CW20 Mint Permissions (Documented)
...
|----------|------|--------|
| **P0** | Fix cancel window: use `WITHDRAW_DELAY` in execute logic instead of hardcoded 300. | ✅ Done |
| **P1** | Restrict `WithdrawCancel` to cancelers only (remove operator/admin). | ✅ Done |
| **P1** | Restrict `SetCustomAccountFee`/`RemoveCustomAccountFee` to admin only. | ✅ Done |
| **P2** | Add tests: full deposit flow (limits, fees), RecoverAsset. | ✅ Done |
| **P2** | Add test for `WithdrawExecuteMint` (wrong token type). | ✅ Done |
| **P2** | Align `InvalidWithdrawDelay` error message with validation range. | ✅ Done |
| **P3** | Document admin-as-operator and canceler-as-separate-role design decisions. | ✅ Done (`docs/OPERATIONAL_NOTES.md`) |
| **P3** | Consider deposit-side rate limits for symmetry. | 📋 Future |
