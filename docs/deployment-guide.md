...
- **Solana mainnet (browser):** Prefer CORS-friendly URLs such as `https://solana-rpc.publicnode.com/`. `api.mainnet-beta.solana.com` and many third-party demo keys often return HTTP 403 or 429 from browser contexts (see issue #109 and `solanaMainnetRpcDefaults.ts`).
