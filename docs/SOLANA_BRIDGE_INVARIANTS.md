...
This document lists security and correctness invariants for the Anchor programs under `packages/contracts-solana/programs/`, with cross-links to automated tests. Governance (admin, operator, canceler keys) is assumed honest unless noted. **UI:** Solana *recipient* strings on the web bridge must be valid ed25519 *on-curve* public keys, not just decodable 32-byte base58 — see [FRONTEND_BRIDGE_INVARIANTS.md](./FRONTEND_BRIDGE_INVARIANTS.md) (**INV-RCP1**), [skills/agent-bridge-recipient-validation.md](../skills/agent-bridge-recipient-validation.md), and issue **117** (on-curve enforcement for address typos that remain valid base58). **Browser wallet tx submission:** see **INV-FE-SOLANA-BH1** in [FRONTEND_BRIDGE_INVARIANTS.md](./FRONTEND_BRIDGE_INVARIANTS.md) and [skills/agent-solana-tx-blockhash.md](../skills/agent-solana-tx-blockhash.md) (GL-128).
