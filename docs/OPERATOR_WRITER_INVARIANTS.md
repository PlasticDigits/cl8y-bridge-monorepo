...
Executed, cancelled, missing (`submittedAt == 0`), and `BelowMinPerTransaction` / Terra `BelowMinimumAmount` are **terminal**. Drop them from `pending_executions` and record them in `terminal_executions` so enumeration does not re-queue. RPC / `CancelWindowActive` / period `RateLimitExceededPerPeriod` failures stay retryable — **do not** add the period-full revert string to the terminal list (GL-170). A payout **above** `maxPerPeriod` or `maxPerTx` (or below min) is not sent; those permanent caps use `PermanentPeriodCapExceeded` / `PermanentPerTxCapExceeded` / `BelowMinPerTransaction` and are terminal. Temporary period-full (`used + amount > maxPerPeriod` while `amount <= maxPerPeriod`) does **not** send and retries at `windowStart + RATE_LIMIT_WINDOW` (`getWithdrawRateLimitWindow`), not at `WRITER_RPC_BACKOFF_MAX`. If that read fails after a period revert, wait one full `RATE_LIMIT_WINDOW` (24h) instead of the short RPC cap. GuardBridge `WithdrawRateLimitExceeded` is the same kind of retry: wait one 24h window, and do not mark that revert terminal.

Retryable failures increment `attempts` and apply backoff from *now* (`execute_queue`); do **not** reset an in-flight timer on enumeration re-queue. `pending_executions.update` refreshes the cache TTL when a retry is scheduled so a long period wait is not hidden from `get` and re-queued at delay 0. On-chain `TokenRateLimit` / min / max stay unchanged. No operator execute bypass.
