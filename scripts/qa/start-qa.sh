...
# Non-default SSH port (e.g. QA host listens on 2223). OpenSSH: ssh -p / scp -P
...
echo "Run (copy-paste as one block). Optional env when running make start-qa on this host:"
echo "  QA_SSH_DEST=user@FIXED_IP   — bake your laptop login (IP or DNS; default is whoami@hostname -f)"
echo "  QA_SSH_PORT=2223            — if SSH is not on port 22 (printed in ssh/scp below)"
echo "  (127.0.0.1 on both sides avoids IPv6 [::1] bind errors on some Linux desktops.)"
echo ""
...
echo "  ${SSH_DEST}"
echo ""
echo "On laptop (same repo): copy only .deploy/local.env from this host, then generate Vite env:"
echo "  mkdir -p .deploy && scp ${SCP_P_ARGS}${SSH_DEST}:${REPO_ROOT}/.deploy/local.env .deploy/local.env"
echo "  ./scripts/qa/write-frontend-env-local.sh && cd packages/frontend && npm ci && npm run dev"
echo "  (or: npm run env:local --prefix packages/frontend)"
