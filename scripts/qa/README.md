...
| **Terra deploy: `bulk memory support is not enabled` / Wasm deserialization** | Modern **rustc** can emit WASM bulk-memory ops; older **wasmd/wasmvm** (e.g. some LocalTerra images) reject them. **`scripts/ensure-cw20-mintable-wasm.sh`** builds cw-plus with **`RUSTFLAGS=-C target-feature=-bulk-memory`** by default. Remove **`cw20_mintable.wasm`** and re-run **`make start-qa`** so it rebuilds. If it still fails, set **`CW20_MINTABLE_RUSTFLAGS`** (e.g. **`-C target-cpu=mvp`**) or delete the artifact and rely on **`scripts/download-cw20-wasm.sh`** (release binary). |
