...
**Token-2022 (T2022):** The Solana test validator started by **`docker compose`** clones the Token-2022 program (`TokenzQd…`) alongside wSOL so **`qa:full-token-setup`** can mint a **T2022** SPL (9 decimals) and register it with Anvil / Anvil1 / Terra. On-chain tickers for the paired **ERC20** and **CW20** use **`TTWT`** (letters only): cw20-base rejects symbols outside **`[a-zA-Z\-]{3,12}`**, so **`T2022`** cannot be used on Terra. **`VITE_*_T2022`** vars still name the row. **Settings → Faucet** does **not** list T2022: **`cl8y_faucet`** registers classic SPL mints only; fund T2022 ATAs manually or via scripts.
