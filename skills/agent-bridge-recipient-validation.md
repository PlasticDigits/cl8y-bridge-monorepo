...
- issue **117** — launch blocker; extended scope covers Terra + EVM + Solana in one recipient pass.
- issue **119** — CTA / receive quote / MAX / amount field UX: **INV-UX1** in `docs/FRONTEND_BRIDGE_INVARIANTS.md` (text + `inputMode=decimal` amount input to avoid native `step` conflicts; excess-precision line shows exact floored `parseAmount` value).
